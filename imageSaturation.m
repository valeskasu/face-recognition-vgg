function imageSaturation( dirName, simulationDir)
...
for i = 1:numel(fileListFullPath)
    if ~isempty(regexpi(fileListName{i},'.jpg')) || ~isempty(regexpi(fileListName{i},'.ppm'))
        ...
        im1 = HSV;
        im1(:,:,2) = HSV(:,:,2)*0.8; %diminuir brilho em 20 por cento
        im1(im1>1)=1;
        im1 = hsv2rgb(im1);
        
%         
        im2 = HSV;
        im2(:,:,2) = HSV(:,:,2)*1.2; %aumentar o brilho em 20 por cento
        im2(im2>1)=1;
        im2 = hsv2rgb(im2);
        
        imwrite(im1,[simulationDir '/' fileListName{i} '-20decreasedSaturation.jpg']);
        imwrite(im2,[simulationDir '/' fileListName{i} '-20inscreasedSaturation.jpg']);
